...
from app.services.mod.mod_content_service import (
    ModContentService,
    get_mod_content_service,
)
from app.services.resourcepack.resourcepack_content_service import (
...
class CompileContentService:
    ...
    async def get_compiled_contents(self, instance_type: InstanceType) -> list[CompiledContentContainer]:
        ...
        for content_service in self.content_services:
            ...
            compiled_content_container = CompiledContentContainer(
    ...
    async def get_contents_hash(self) -> str:
        content_hash = ""
        for content_service in self.content_services:
            content_hash += await content_service.get_contents_hash()
        return content_hash


async def get_compile_content_service(
    mod_content_service: ModContentService = Depends(get_mod_content_service),
    resourcepack_content_service: ResourcepackContentService = Depends(get_resourcepack_content_service),
